      ...
      <main className="max-w-2xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm px-6 py-5">
          <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-sm text-gray-500">{doneCount} de {sessions.length} sesiones</p>
          </div>
          <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-gray-900 transition-all" style={{ width: `${completionPct}%` }} />
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-base font-semibold text-gray-900">Fases</h2>
            <AddPhaseForm action={addPhaseAction} />
          </div>
          {phases && phases.length > 0 ? (
            <ul className="space-y-2">
              {phases.map((phase) => (
                <li key={phase.id} className="bg-white border border-gray-100 rounded-2xl shadow-sm px-5 py-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-400">· {phase.session_count} sesiones</span>
                    </div>
                    <form action={deletePhase.bind(null, phase.id, id)}>
                      <button type="submit" className="text-xs text-gray-300 hover:text-red-500 transition-colors">
                        Eliminar
                      </button>
                    </form>
                  </div>
                  {phase.objectives && (
                    <p className="text-xs text-gray-500 mt-2">{phase.objectives}</p>
                  )}
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-sm text-gray-400">Sin fases todavía. Puedes trabajar sesión por sesión directamente.</p>
          )}
        </div>

        {/* Sessions timeline */}
        <div>
          <h2 className="text-base font-semibold text-gray-900 mb-3">Sesiones del plan</h2>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-medium text-gray-400 w-6">#{s.session_number}</span>
                    {phase && (
                      <span
                    ...
                    <div>
                      {s.session_id ? (
                        <Link href={`/sessions/${s.session_id}`} className="text-sm text-gray-900 hover:underline">
                          {phase ? phase.title : 'Sesión vinculada'}
                        </Link>
                      ) : (
                        <p className="text-sm text-gray-600">{phase ? phase.title : 'Sin asignar'}</p>
                      )}
                  ...
                  <div className="flex items-center gap-2">
                    {s.status === 'planned' && (
                      <form action={markPlanSessionSkipped.bind(null, s.id, id)}>
                        <button type="submit" className="text-xs text-gray-300 hover:text-amber-600 transition-colors">
                          Saltar
                        </button>
                      </form>
                    )}
